...
const PATTERNS = [
  ...
  },
  {
    ...
    risk: 'HIGH',
    terms: ['ngân hàng', 'tài khoản bị khóa', 'xác minh', 'đăng nhập', 'cập nhật', 'sinh trắc học', 'thẻ tín dụng'],
  ...
  },
  {
    ...
    category: 'Giả mạo cơ quan',
    risk: 'CRITICAL',
    terms: ['công an', 'tòa án', 'viện kiểm sát', 'tài khoản tạm giữ', 'vụ án', 'rửa tiền', 'ma túy', 'lệnh bắt'],
    signals: ['giữ bí mật', 'chuyển tiền', 'cung cấp thông tin', 'đe dọa'],
...
function buildBackendFile() {
  const code = \`// Tự động tạo bởi build-patterns.js
const SCAMCHECK_RAG_VERSION = '\${VERSION}';

const SCAM_REFERENCE_CORPUS = \${JSON.stringify(PATTERNS, null, 2)};
...
\`;
  fs.writeFileSync(path.join(__dirname, 'lib', 'rag-corpus.js'), code);
}

function buildExtensionFile() {
  const offlinePatterns = PATTERNS.map(p => ({
    id: p.id,
    title: p.title,
    risk: p.risk,
    terms: [...p.terms, ...p.signals]
  }));

  const code = \`// Tự động tạo bởi build-patterns.js
(function (scope) {
  const VERSION = '\${VERSION}';
  const PATTERNS = \${JSON.stringify(offlinePatterns, null, 2)};
...
\`;
  fs.writeFileSync(path.join(__dirname, 'scamcheck-extension', 'offline-patterns.js'), code);
}

buildBackendFile();
buildExtensionFile();
console.log('Successfully built RAG corpuses.');
